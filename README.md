...
Business + data-science background. I build at the seam where **strategy meets a working product** — where an analysis stops being a notebook and becomes a system someone actually runs. My work spans **life sciences, cybersecurity, supply chain, energy, HR, finance, and sports** — same craft, different domain.

- 🌍 Lebanon · UAE · GCC · Europe
- 💼 Strategy · Product · Data Analytics · Applied AI
- 🎯 Interested in: process intelligence, operational AI copilots, computational biology & fintech
...
|---|---|---|
| 🟢 **[Genetic Variant Triage](https://jadzoghaib.github.io/genetic-variant-triage/)** · [code](https://github.com/jadzoghaib/genetic-variant-triage) | Thousands of genetic variants sit unclassified — not benign, not pathogenic, just *uncertain*. Reconciles AlphaMissense's predictions against ClinVar's clinical assertions on AlphaFold structure, then grades how much evidence actually backs each call. Rebuilds weekly against live ClinVar | `AlphaFold` `AlphaMissense` `ClinVar` `DuckDB` `3Dmol.js` |
| ↳ **[Illustrated manual](https://jadzoghaib.github.io/genetic-variant-triage/manual.html)** | The problem, the data sources, and how the triage actually works — written for a reader who has never seen a variant table | `SVG` `Explainer` |
| 🟡 **[Batch Investigation Console](https://jadzoghaib-batch-traceability-root-cause-studio-for--app-zrys7k.streamlit.app/)** · [code](https://github.com/jadzoghaib/Batch-Traceability-Root-Cause-Studio-for-Pharma-Manufacturing) | Traces tablet quality back to raw-material attributes and compression conditions across **1,005 real production batches**. Built around one finding: 22 of 44 candidate drivers *reverse their correlation sign* pooled vs. within a peer cohort — so every comparison is cohort-scoped | `Python` `Streamlit` `pandas` `RCA` |
| 🟢 **[EnzySelect](https://enzyselect.streamlit.app/)** · [code](https://github.com/jadzoghaib/enzyselect) | Prioritizes candidate enzymes for PET plastic degradation against operating conditions and a testing budget — transparent, decomposable scoring with real AlphaFold/UniProt structural anchors. Synthetic candidate set, and the app says so on every screen | `Python` `Streamlit` `AlphaFold` `UniProt` |
...
| ↳ **[Documentation hub](https://jadzoghaib.github.io/aegis-soc-triage/)** | Four references: the interactive threat manual (14 techniques mapped to MITRE ATT&CK, each with a clickable architecture diagram), the six workflow specs, the ontology, and the incident lifecycle | `SVG` `ATT&CK` |
...
| Project | What it does | Stack |
|---|---|---|
| **[OilShield](https://github.com/jadzoghaib/oilshield)** | Strategic stress-testing console for oil & gas portfolios: build a company from typed assets, shape a 36-month Brent scenario, watch break-evens, liquidity headroom and shut-in economics recompute. Revenues float with price while fixed costs keep running — that asymmetry is the product | `Python` `Streamlit` `Plotly` |
...
| Project | What it does | Stack |
|---|---|---|
| **[Stride](https://github.com/jadzoghaib/stride)** | Athlete monetization and sponsorship platform: athletes own evidence-based marketability analytics, sponsors match campaign briefs with fully decomposable scoring, deal pipeline runs end to end | `FastAPI` `React` `Supabase` |
